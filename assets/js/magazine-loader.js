(function($) {
    window.initMagazineContainer = function(container) {
        ...
        .then(function(response) {
            ...
            return $.ajax({
                url: data.restUrl + "magazines/" + response.shortcode.list_type + "?_=" + timestamp,
                method: "GET",
                cache: false,
                headers: { 
                    "X-WP-Nonce": data.nonce,
        ...
        .then(function(data) {
            ...
            if (containerClass) {
                ...
                var containerStyles = cssRules.map(function(rule) {
                    if (!rule.trim()) return '';
                    ...
                    if (selector.includes('&')) {
                        ...
                        return selector.replace('&', '.' + containerClass) + '{' + styles + '}';
                    } else {
                        // Add container class as parent
                        return '.' + containerClass + ' ' + selector + '{' + styles + '} ';
                    }
                }).join('\n');
            ...
            if (data.shortcode.list_type !== "featured") {
                wrapperClasses.push(data.shortcode.display_type === "scroll" ? "magazine-scroll" : "magazine-grid");
            }
            if (containerClass) {
        ...
        .fail(function(error) {
            ...
            $container.html("<p>Error loading content. Please check browser console for details.</p>");
        });
    };
